...
class DiceSpec extends AnyWordSpec {
  ...
  "A Dice" should {
    val wu = new Dice()
    "be created using the default constructor without any parameters" in {
    ...
    "be rolled using 1 or 2 as the number of dices to use" in {
      for (n <- (0 to 100)) {
        wu.wuerfeln(1)
        wu.getSum() should be <= 6
        wu.toString should fullyMatch regex """[0-6]\sund\s0"""
      }
      for (n <- (0 to 100)) {
        wu.wuerfeln(2)
        wu.getSum() should be <= 12
        wu.toString should fullyMatch regex """[0-6]\sund\s[0-6]"""
      }
    }
